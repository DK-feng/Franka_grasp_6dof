...
def connected_components(Z, epsilon, metric='cosine'):
    ...
    for i in range(n):
        if cluster_labels[i] == -1:
            ...
            if torch.unique(cluster_labels[component_seeds.to(cluster_labels.device)]).shape[0] > 1:
                #temp = cluster_labels[component_seeds].numpy()
                temp = cluster_labels.to(component_seeds.device)[component_seeds].cpu().numpy()
...
def mean_shift_smart_init(X, kappa, num_seeds=100, max_iters=10, metric='cosine'):
    ...
    cluster_labels = seed_cluster_labels.to(closest_seed_indices.device)[closest_seed_indices]
    #print(f"seed_cluster_labels.device: {seed_cluster_labels.device}, closest_seed_indices.device: {closest_seed_indices.device}")
