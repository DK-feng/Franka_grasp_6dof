...
def test(model, loader, num_class=40, vote_num=1):
    ...
    for j, (points, target) in tqdm(enumerate(loader), total=len(loader)):
        ...
        print(type(points))
        print(points.shape)  #[batch_size, 1024, 3]
        for i in range(points.shape[0]):
            visualize_point_cloud([points[i].cpu().numpy()])
            points_x = points[i,:,0]
            points_y = points[i,:,1]
            points_z = points[i,:,2]
            print("batch:{}, max_x:{:.2f}, min_x:{:.2f}, max_y:{:.2f}, min_y:{:.2f}, max_z:{:.2f}, min_z:{:.2f}".format(
                i,torch.max(points_x),torch.min(points_x),torch.max(points_y),torch.min(points_y),torch.max(points_z),torch.min(points_z)
            ))
        time.sleep(1000000)
