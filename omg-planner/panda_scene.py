...
class PandaYCBEnv:
    ...
    def cache_objects(self):
        ...
        '''
        data/objects/002_master_chef_can
        data/objects/003_cracker_box
        data/objects/004_sugar_box
        data/objects/005_tomato_soup_can
        data/objects/006_mustard_bottle
        data/objects/010_potted_meat_can
        data/objects/019_pitcher_base
        data/objects/021_bleach_cleanser
        '''
...
if __name__ == "__main__":
    ...
    # ) 
    parser.add_argument(
        "-d", "--dir", help="filename", type=str, default="app/shared_dir/"
    ) 
    ...
    scene_file = args.dir + args.file
    print(scene_file)
    scene = sio.loadmat(scene_file)
    scene_file = args.file
    all_objs = list(scene["used_objects_index"][0])
    env = PandaYCBEnv(renders=args.vis, egl_render=args.egl, all_objs=all_objs)
    ...
    for scene_file in scene_files:
        ...
        full_name = os.path.join('app/shared_dir/', scene_file + ".mat")
        ...
        print(type(plan))
        print(plan.shape)
        col_0_7 = np.round(plan[:, :7], 3)
        col_7_10 = np.round(plan[:, 7:], 2)
        plan = np.hstack((col_0_7, col_7_10))
        np.set_printoptions(suppress=True, precision=3)
        print('--------------------')
        for single_step in plan:
            print(single_step)
        print('--------------------')
